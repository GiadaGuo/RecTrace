...
flowchart TD
    ...
    subgraph FlinkLayer ["Flink 处理层 (Java，现有模块改造)"]
        ...
        Job2["Job2: WindowAggJob\nhome 漏斗 1min 窗口聚合\nCTR/CTB/CVR → Kafka + Redis feat:user_item:*"]
    ...
    subgraph FrontendLayer ["前端层 (新建，React 18 + TypeScript)"]
...
| `商品详情页收藏` | `bhv_id = '3001', bhv_page = 'pdp', bhv_type = 'click', bhv_src = 取透传值, bhv_value = 'fav',bhv_ext = {"item_id": "I0000456","req_id":  "req_abc12345"}` 
| `商品详情页加购` | `bhv_id = '3001', bhv_page = 'pdp', bhv_type = 'click', bhv_src = 取透传值, bhv_value = 'cart',bhv_ext = {"item_id": "I0000456","req_id":  "req_abc12345"}` 
| `商品详情页购买` | `bhv_id = '3001', bhv_page = 'pdp', bhv_type = 'click', bhv_src = 取透传值, bhv_value = 'buy',bhv_ext = {"item_id": "I0000456","req_id":  "req_abc12345"}` 
...
feat:item:{item_id}     -> Hash  { total_detail_click, total_buy, total_cart, total_fav,
                                   conversion_rate, uv, item_brand, item_price, item_title, category_id }
                                   # total_detail_click：商品被点击进入详情页的次数（bhv_type=click, bhv_value=null）
                                   # conversion_rate = total_buy / total_detail_click（点击转化率）
...
# 用户×类目短期窗口特征（Job2 WindowAggJob 写入，home 推荐漏斗，1分钟滚动窗口，覆盖写）
# 事件归因范围：
#   show_cnt  ← bhv_page=home, bhv_type=show（Job1 已为 home show 拼接商品维度）
#   click_cnt ← bhv_page=home, bhv_type=click, bhv_value=null
#   cart/fav/buy ← bhv_page=pdp AND bhv_src=home（从 home 推荐路径进入 PDP 后的转化）
feat:user_item:{uid}:{catId} -> Hash  { show_cnt, click_cnt, cart_cnt, fav_cnt, buy_cnt,
                                        ctr, ctb, cvr, window_start, window_end }, TTL 600s
                                        # CTR = click_cnt / show_cnt   （曝光→点击，推荐曝光质量）
                                        # CTB = buy_cnt   / click_cnt  （点击→购买，精排信号）
                                        # CVR = buy_cnt   / show_cnt   （曝光→购买，整体转化效率）
                                        # 覆盖写语义：每个窗口关闭后 HSET 覆盖旧值，反映最近一个完整1分钟窗口
                                        # TTL 600s（10分钟）：用户不活跃时自然过期

# 用户×商品交叉特征（Job3 CrossFeatureFunction 写入，指数衰减计数）
# 衰减模型：Exponential Moving Count，半衰期 7 天
#   new_count = old_count * exp(-ln2 * Δt / T_half) + 1
#   Δt 为同一 (uid, catId/brand) 的相邻事件时间差，T_half = 7 * 86400 * 1000 ms
# 语义：反映用户对该品类/品牌的当前兴趣强度，而非原始历史点击累计
#       兴趣迁移（Interest Drift）后旧信号自然衰减，不压制新兴趣
cross:{uid}:{category_id} -> Hash  { pv_cnt, click_cnt, cart_cnt, fav_cnt, buy_cnt, last_ts }, TTL 30d
cross:{uid}:{brand}        -> Hash  { click_cnt, buy_cnt, last_ts }, TTL 30d
                              # 所有数值字段为衰减后的浮点数（保留4位小数）
...
Job1 DimJoinJob         : ODS → DWD（AsyncIO Redis 维度拼接，透传新字段）
                          home show 事件额外拼接商品维度（brand/price），为 Job2 漏斗计算提供完整数据
Job2 WindowAggJob       : DWD[home 漏斗] → dws_user_item_feature + Redis feat:user_item:*
                          （1分钟滚动窗口，show+click 来自 bhv_page=home，转化来自 bhv_src=home）
                          指标：show_cnt/click_cnt/cart_cnt/fav_cnt/buy_cnt + CTR/CTB/CVR
...
Job3 RealtimeFeatureJob : DWD → Redis/dws_user_feature/dws_item_feature（特征挖掘层）
  ...
  └── ItemFeatureFunction    : 全局累计计数（detail_click/buy/cart/fav）+ UV + 点击转化率
  └── WindowStatFunction     : 近5min/1h 窗口统计（用户/商品维度，TTL State 实现滑动窗口）
  └── CrossFeatureFunction   : 用户×品类、用户×品牌 指数衰减交叉计数（EMC，半衰期7天）→ cross:{uid}:{cat/brand}（新增）
...
│                                                                   │
│  商品统计特征：                                                   │
│    feat:item:{item_id} → 全局累计：total_detail_click/buy + UV + 点击转化率 │
...
Stage 2：曝光 × 点击流 Join（10 分钟窗口）
  sample_impression_home × dwd_behavior_with_dim(bhv_type=click)
      ...
      ↓  输出：{ req_id, uid, item_id, show_ts, label_click, click_ts, ... }
...
Stage 3：点击样本 × 加购/购买行为 Join（20 分钟窗口）
  sample_with_click_label(label_click=1) × dwd_behavior_with_dim(bhv_type=cart/buy, bhv_src=home)
      ...
      ↓  窗口：点击发生后 20 分钟
      ├── 20分钟内有 cart/buy → label_cart=1 / label_buy=1，记录 cart_ts / buy_ts
      └── 20分钟内无 cart/buy → label_cart=0 / label_buy=0，cart_ts / buy_ts = null
      ↓  输出最终训练样本：{ req_id, uid, item_id,
                             show_ts, click_ts, cart_ts, buy_ts,
                             label_click, label_cart, label_buy, features }
...
| 字段 | 含义 | 来源 |
|------|------|------|
| `show_ts` | 商品曝光时间戳（ms） | Stage 1 曝光事件 `ts` |
| `click_ts` | 点击发生时间戳（ms），未点击为 null | Stage 2 Join 命中时取 click 事件 `ts` |
| `cart_ts` | 加购时间戳（ms），未加购为 null | Stage 3 Join 命中时取 cart 事件 `ts` |
| `buy_ts` | 购买时间戳（ms），未购买为 null | Stage 3 Join 命中时取 buy 事件 `ts` |

**保留时间戳的价值**：
- **科学设置窗口**：离线分析 `click_ts - show_ts` 的 P90/P95 分布，替代人工经验拍脑袋定窗口
- **响应时间特征**：`click_ts - show_ts`（决策速度）可作为独立特征输入模型，决策越快意图信号越强
- **事后重新打标**：修改窗口参数后可直接对 PostgreSQL 中的样本重跑标注逻辑，无需重放 Kafka

**时间窗口的灵活配置原则**：
- 主页推荐普通商品：点击窗口 10 分钟，转化窗口 20 分钟（根据 `click_ts - show_ts` 分布 P90 调整）
...
TOOL_SPECS = [
    ...
    },
    {
        ...
        "parameters": {
            "feature_name": {"type": "string", "description": "特征名，如 total_detail_click、conversion_rate"}
        },
...
### 6.6 记忆层设计（ChromaDB + RAG，Phase 2）
...
长期记忆不替代 `tools.py` 的实时工具调用，而是作为 **检索增强生成（RAG）** 的辅助层，专门解决两类问题：
- **经验复用**：从历史成功诊断案例中检索相似问题，给当前问题提供 few-shot 示例
- **模式补充**：沉淀常见异常模式（如 Kafka lag 激增、特征漂移、Redis miss、排序日志缺失），帮助 Agent 在模糊问题下更快收敛

**核心原则**：
- **事实问题靠工具**：如“为什么推荐这个商品”“某商品在哪一层被过滤”，必须调用 `get_rec_snapshot`、`get_feature_contributions`、`get_ranking_trace` 等实时工具
- **经验问题靠 RAG**：如“最近推荐变差通常先查什么”“这种现象更像漂移还是延迟”，先检索相似案例，再决定优先调用哪些工具
- **RAG 只增强推理，不直接生成最终事实答案**：检索结果作为提示上下文，不能替代 Redis / ClickHouse 返回的实时数据

**RAG 的触发场景**：

| 问题类型 | 是否先做 RAG 检索 | 原因 |
|---------|------------------|------|
| 明确解释类（含 `req_id` / `item_id`） | 否 | 直接走 Workflow，调用实时工具即可 |
| 模糊诊断类（“推荐变差了”“点击率下降了”） | 是 | 先检索相似异常案例，缩小排查范围 |
| 特征溯源类（需要多步级联） | 可选 | 当历史上存在相似根因路径时，用 few-shot 提升工具选择稳定性 |

**ChromaDB 中存储的案例结构**：

每条案例不是简单问答，而是“问题 + 工具路径 + 根因 + 修复动作”的结构化诊断单元：

```json
{
  "question": "最近推荐点击率下降，但曝光量正常，如何排查？",
  "intent_type": "pipeline_diagnosis",
  "symptoms": ["ctr_down", "impression_stable"],
  "tools_used": ["get_user_rec_quality", "get_pipeline_metrics", "check_feature_drift"],
  "root_cause": "item_price 特征发生分布漂移",
  "resolution": "重刷离线特征并回填 Redis 热 key",
  "confidence": 0.92
}
```
...
用户问题
   ↓
入口路由（明确 req_id/item_id ?）
   ├── 是 → 直接走 Workflow / ReAct 工具链
   └── 否 → 先调用 memory.retrieve_similar_cases(query)
                ↓
             取 Top-K 相似案例
                ↓
             将 “历史案例摘要 + 推荐工具路径” 拼入 system/context
                ↓
             LLM 基于增强上下文决定下一步调用哪个工具
                ↓
             tools.py 返回实时事实数据
                ↓
             汇总生成最终回答
...
RAG 检索应位于 `call_model` 之前的轻量预处理节点，仅在模糊诊断任务中触发：

```python
# recommend-service/agent/orchestrator.py（Phase 2）
def retrieve_cases(state: AgentState):
    query = state["user_query"]
    if state.get("intent_type") != "pipeline_diagnosis":
        return {"retrieved_cases": []}
    cases = memory.retrieve_similar_cases(query, top_k=3)
    return {"retrieved_cases": cases}

# 路由：retrieve_cases -> agent -> tools -> agent
```

这样设计的好处是：
- 明确解释类问题不增加额外延迟
- 模糊诊断类问题先注入“历史工具使用路径”，提升工具选择准确率
- RAG 与现有 `Workflow / ReAct` 架构正交，不破坏 `tools.py` 的实时查询边界

**ChromaDB memory.py 示例**：
...
│   └── memory.py            # 长期记忆 ChromaDB + RAG 封装（Phase 2）
└── routers/
...
- [ ] 改造 `job3/RealtimeFeatureJob.java`（**特征挖掘层**）
  ...
  - 新增 `CrossFeatureFunction`：KeyBy `uid`，维护 `cross:{uid}:{cat}` 和 `cross:{uid}:{brand}` 指数衰减交叉计数 Hash（EMC，半衰期7天，Double State，每次写入前施加衰减再 +1）
...
- [ ] 新建 `job6/SampleJoinJob.java`
  ...
  - 最终样本：`{ req_id, uid, item_id, show_ts, click_ts, cart_ts, buy_ts, label_click, label_cart, label_buy, features }` → `sample_final_home`
...
- [ ] `agent/memory.py`：长期记忆 ChromaDB + RAG 封装，历史诊断案例检索与案例沉淀（Phase 2）
...
### M7 前端（新建 `frontend/`，React 18 + TypeScript + ECharts）
...
│       ├── job2/WindowAggJob.java          # home 渠道 1min 滚动窗口聚合 → Kafka + Redis
...
│   │   ├── lineage.py
│   │   └── memory.py                  # Phase 2：ChromaDB + RAG 长期记忆
│   ├── olap/
...
│   └── requirements.txt
│
├── frontend/                         # 新建，React 18 + TypeScript（M7）
│   ├── src/
│   │   ├── pages/
│   │   │   ├── Playground.tsx        # 页面二，MVP
│   │   │   ├── AgentChat.tsx         # 页面三，MVP
│   │   │   └── Monitor.tsx           # 页面一，Phase 2
│   │   └── api/
│   │       └── recommend.ts
│   ├── package.json
│   └── Dockerfile
...
2f. Job3 新增 CrossFeatureFunction（uid×categoryId、uid×brand 指数衰减交叉计数，EMC 半衰期7天）
```
...
```
实现顺序（内部拆分）：注意要在当前虚拟环境下进行，先source .venv/bin/activate
3a. main.py + routers/recommend.py（POST /recommend 接口骨架）
3b. feature/feature_fetcher.py（从 Redis 读 feat:user / feat:item / seq:click）
3c. models/rule_model.py（加权规则打分，无需训练）
3d. feature/snapshot_writer.py（写 snapshot:rec:{req_id}，为 Agent 溯源做准备）
3e. POST /recommend/inject（注入行为后触发特征重算）
```
...
| P2-10 | M5 Agent 长期记忆 + RAG | P2-6 稳定后 | ChromaDB 历史诊断案例检索 + few-shot 提示增强 + 推荐工具路径复用 |

**Phase 2 各模块单测补充要求**：

| 模块 | 测试重点 |
|------|---------|
| M2.5 SampleJoinJob | Flink 本地测试：曝光事件 + 10min 内 click → label_click=1，click_ts 非 null 且等于 click 事件 ts；超出窗口无 click → label_click=0，click_ts=null；buy 事件 bhv_src≠home 时不拼接 label_buy；show_ts 始终等于曝光事件 ts |
